...
class _FridaToolState extends State<FridaTool> {
  ...
  Future<void> _loadStatus() async {
    ...
    try {
      final (frida, info) = await (widget.repository.getFridaStatus(), widget.repository.getFridaInfo()).wait;
      if (mounted) setState(() { fridaStatus = frida; fridaInfo = info; });
    } catch (e) {
  ...
  List<_StatusItem> _getStatusItems() {
    ...
    if (fridaStatus != null) {
      items.addAll([
        _StatusItem('Status', fridaStatus!.isRunning ? 'Running' : 'Stopped', fridaStatus!.isRunning ? Colors.green : Colors.orange),
    ...
    if (fridaInfo != null) {
      items.addAll([
        ...
        _StatusItem('Needs Update', fridaInfo!.needsUpdate ? 'Yes' : 'No', fridaInfo!.needsUpdate ? Colors.orange : Colors.green),
  ...
  List<_ActionButton> _getActions() {
    ...
    return [
      if (!installed) _ActionButton('Install', Icons.download, isLoading ? null : _installFrida),
      if (installed && !running) _ActionButton('Start', Icons.play_arrow, isLoading ? null : _startFrida),
      if (running) _ActionButton('Stop', Icons.stop, isLoading ? null : _stopFrida),
      if (needsUpdate) _ActionButton('Update', Icons.update, isLoading ? null : _updateFrida),
    ];
  }
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              title: Text('Frida', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
                      ...
                      ? GestureDetector(
                          onTap: () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Path copied to clipboard'), duration: Duration(seconds: 2)),
                            );
                          },
                          child: Row(
                            children: [
